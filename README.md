...
- **Python 3.9+**
- Windows / macOS / Linux
- 百度翻译 API 凭据（免费额度足够日常使用）

### 1. 获取百度 API 凭据

详见 **[百度大模型翻译 API 申请与配置指南](docs/baidu-api-setup.md)**

1. 访问 [百度翻译开放平台](https://fanyi-api.baidu.com/)
2. 注册/登录，开通 **「大模型文本翻译API」** 服务
3. 在控制台获取 `APP ID` 和 `密钥`

> ⚠️ ** `config.yaml` 中的 `app_id` 和 `secret_key`**，每位玩家需自行申请。免费额度每月 100 万字符，足够日常使用。
...
### Q: 翻译不生效？
A: 检查：
1. `translation.enabled` 是否为 `true`
2. `baidu.app_id` 和 `baidu.secret_key` 是否正确
3. 百度账户是否有余额、是否已开通「大模型文本翻译API」
4. 控制台是否有 `翻译API错误` 日志
...
### Q: 数据库文件越来越大？
A: SQLite 缓存以 key-value 存储翻译对，每个条目几十到几百字节。正常游戏数周后约几 MB。如需清理，删除 `translation_cache.db` 即可（全新重建）。
...
├── translation_cache.db     # SQLite 磁盘翻译缓存（自动生成）
├── README.md
│
├── src/
│   ├── main.py              # 入口：初始化配置、翻译器、Web 服务器
│   ├── config_loader.py     # config.yaml 读取与校验
│   ├── mud_client.py        # Telnet 客户端：连接游戏、过滤 MCCP、行处理
│   │                        #   核心翻译逻辑：ANSI 段拆分 → 逐段翻译 → 重建
│   ├── translator.py        # 百度大模型翻译 API 客户端
│   │                        #   PersistentCache: SQLite 磁盘缓存
│   │                        #   LRUCache: 内存 LRU 缓存
│   ├── parser.py            # 文本流解析器：Telnet IAC、ANSI 提取、Prompt 检测
│   ├── ansi_html.py         # ANSI 转义序列 → HTML 转换器
│   └── web_server.py        # aiohttp Web 服务器：静态文件 + WebSocket 通信
│
└── static/
    ├── index.html           # 前端页面
    ├── app.js               # WebSocket 客户端 + 终端渲染 + 调试面板
    └── style.css            # 暗色终端主题 + 16 色 ANSI 样式 + 调试面板样式
```
...
| `config_loader.py` | 读取 `config.yaml`，提供类型化配置属性 |
